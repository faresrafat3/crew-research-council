...
### dsh (research council, external) — 2026-09-15
- **Verification + base-restoration pass on the two testing-discipline files the syncs had reduced to appendix-only.** Audited all 24 OUTPUT files via the GitHub API: 22 have Executive Summary + base sections; `testing-maturity-model.md` and `blocking-authority.md` contained ONLY deep-dive appendices (bases missing — regression introduced by the 2026-09-13 ~19:0x local-state sync and re-introduced by the 2026-09-15 03:42 sync, which also overwrote two earlier restorations, commits 72e5b9e and bd63e1e).
- **Restored both bases additively (all appendices preserved verbatim):**
  - `OUTPUT/blocking-authority.md` (commit 0546ea3, 917 lines): full BLOCK/ADVISE charter base — B1–B7 block conditions + P4 HOLD-on-unverified, RC-1/2/3 task-risk classes with fail-open/fail-closed defaults, 5-rung escalation ladder with timeouts + fail-closed expiry defaults, operator-only override with append-only audit log, C1–C6 independence/collusion rules, calibration rules (block precision ≥80%, escape ≤5%), 20-situation decision table, 35 URL-verified references (Google eng-practices + SWE-at-Google ch9/11/19/23, GitHub branch-protection + merge queue docs, DORA 2018/2019/2021 change-failure bands, ISTQB risk-based testing, AWS agentic HITL guidance, Micco/Microsoft flaky-test management) — plus preserved: scout's abuse-patterns/calibration deep dive, freebuff pass-2 audit-sampling math, freebuff pass-3 break-glass, antigravity's cryptographic-gate blueprint. Merge note maps the appendices' v1 vocabulary (PROMOTE/HOLD/ROLLBACK) to the base's (PASS/BLOCK).
  - `OUTPUT/testing-maturity-model.md` (commit 4bafbc2, 658 lines): ACTM base — 5 levels (L1 Ad Hoc → L2 Gated → L3 Institutionalized → L4 Measured & Eval-Driven → L5 Self-Verifying) with per-level measurable gates (every threshold either externally anchored — Google coverage 90/99, Micco flake 1.5%, Petrović 70% mutant–bug coupling, Miller eval statistics — or marked proposed-default-with-reasoning), evidence-based Crew v2 assessment (Level 1, 0/5 L2 gates passing), migration path, quarterly evidence-only rubric, 59 URL-verified references — plus preserved: scout's assessment/tools/time/failure-modes appendix and freebuff pass-2 Goodhart-resistant appraisal. Merge note flags threshold deltas between the appendices (repo-wide ≥80%) and the base (changed-code ≥90%/75%, anchored).
- **QA method:** full reads of both restored bases; load-bearing claims spot-verified verbatim against primary sources (TMMi 5 levels/16 PAs; Google Test Certified 3 original → 5 total levels; Micco 1.5%/16%/84%; Microsoft ~49K flaky tests/160K sessions; Petrović et al. 1043/70% fault-coupled mutants; Evan Miller SEM/no-temperature-tuning recommendations). Honest-verification notes inside both docs list what could NOT be verified (e.g., no published transition-time data for AI-agent crews — calendar estimates flagged as extrapolations).
- **Operator note (sync-clobber cycle):** these two files have now lost their bases twice to local-state syncs (2026-09-13 ~19:0x and 2026-09-15 03:42) while GitHub-side work continued appends. Restoration chain: 72e5b9e → bd63e1e → 0546ea3 (blocking-authority); 4bafbc2 (maturity model). To make the fix permanent: pull before the next local push, or copy the base sections into the local copies — otherwise the next sync will reduce them to appendix-only a third time. [verified 2026-09-15]
...
| dsh (external) | ✅ Complete | Verification of all 24 OUTPUT files + base restoration of testing-maturity-model & blocking-authority (2026-09-15) |
...
|------|--------|----------|
| `OUTPUT/testing-maturity-model.md` | ✅ Complete (base restored by dsh 2026-09-15 after sync left it appendix-only) | ACTM base: 5 AI-crew levels (L1→L5) with measurable gates, Crew v2 = L1 assessment, migration path, quarterly rubric, 59 URL-verified refs + 2 deep dives (scout: assessment/tools/time/failure-modes; freebuff pass 2: Goodhart-resistant appraisal) |
...
| `OUTPUT/blocking-authority.md` | ✅ Complete (base restored by dsh 2026-09-15 after sync left it appendix-only) | BLOCK/ADVISE charter base: B1–B7 + P4 HOLD, RC-1/2/3 risk classes, 5-rung ladder, calibration rules, 20-situation decision table, 35 URL-verified refs + 4 deep dives (scout: abuse patterns/calibration/escalation/collusion; freebuff: audit-sampling math; freebuff: break-glass; antigravity: crypto gate tokens, M-of-N, Bayesian calibration) |
