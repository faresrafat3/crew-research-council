...
### Research Council Member (cline) — Deep Dive cycle 4 (tester-soul production mutation) — 2026-09-13
- **Found tester-soul already carried a cycle-3 deep dive (freebuff: oracle quality, 5-class weak-assertion lint, determinism harness).** Appended a second, complementary `[DEEP DIVE] (cline, 2026-09-13)` (104→180 lines) — extends the mutation gate with production-deployment evidence and adds 2026 counter-evidence the base spec does not account for:
  - **Google production mutation playbook** (Petrović et al., ICST 2021 / arXiv:2103.07189; Google Testing Blog 2021-04-12): 14,730,562 mutants across 662,584 changes in 10 languages; change-based findings (≤7 mutants/file) instead of project-wide scores; covered-lines-only, 1 mutant/line, suppression rules, per-line operator selection from {AOR, LCR, ROR, UOI, SBR}. Tester addition #1: report surviving mutants as concrete test goals in `FIX_REQUIRED`, not a scalar `MUT=%`.
  - **Causal evidence for the mutation gate**: exposure to mutant findings correlates with more and *stronger* tests (interventional study vs coverage-only control); 70% (1,043/1,502) of high-priority bugs are coupled to a live mutant on an already-covered bug-introducing change; but at matched suite sizes mutation score weakly correlates with real-fault detection (Papadakis et al., IEEE TSE 2018) — keep ≥70% targeted, never project-wide scalar KPI (harmonizes with quality-metrics.md Cycle 3).
  - **Meta ACH** (Harman et al., FSE 2025 / arXiv:2501.12862; Meta Engineering 2025-09-30): mutant-guided LLM test generation at scale — 9,095 mutants / 571 privacy tests over 10,795 Kotlin classes; engineers accepted 73%; equivalent-mutant agent 0.95/0.96 preprocessed. Tester additions #2/#3: strengthen-loop tests derive from surviving mutants; equivalent-mutant triage before computing `MUT=`.
  - **2026 counter-evidence on agent-written tests** (Chen et al., arXiv:2602.07900v2): six LLMs on SWE-bench Verified — test-writing frequency does not differ between resolved/unresolved tasks, print statements dominate assertions, prompt-induced volume changes do not move outcomes; GPT-5.2 writes almost no tests yet matches top agents. Tester addition #4: engineer on-the-fly tests are observational feedback, never proof — only tester-authored RED-witnessed REQ-marked tests count.
  - **Suite-evolution hazards** (Haroon et al., arXiv:2603.23443: 8 LLMs / 22,374 variants — pass rate 79%→66% under semantic-altering changes, >99% of failures are SPC-residual; Ojdanić et al., arXiv:2212.11762: ~52% of 143,500 mutants degrade across releases, long-standing mutant suites give 10x relevance). Tester additions #5/#6: never accept stale suite runs + flag behavior-frozen SPC-residual tests; persist a long-standing-mutant baseline per module for the Phase 3→4 trend.
  - **Formation boundary** (Spotify honeycomb, 2018): pyramid mix is task-scoped; multi-service tasks switch the suite-mix gate profile.
  - Plus: KB escape-classification template (weak/missing/no-operator taxonomy from Google's non-coupled-bug analysis) filling the ledger taxonomy gap, `MUTF=<shown>/<acted>` verdict field, 10-row calibration table.
- **Method:** tinyfish CLI search + `tinyfish fetch content get` full-text extraction (primary; exa chain in cooldown) + fetch_web_content fallback. All sources fetched 2026-09-13: arXiv:2103.07189 (full HTML), arXiv:2501.12862 (abstract), Meta Engineering ACH post, Google Testing Blog mutation post, arXiv:2602.07900v2, arXiv:2603.23443, arXiv:2212.11762, Spotify Testing of Microservices, Wikipedia GPT-5.2. [verified: 2026-09-13]
- **Gaps / could not verify:** local message_agent() verdict schema, classifier thresholds, tool version pins — same operator-side gaps as prior cycles (flagged in Remaining Gaps).
...
#### Attribution (deep dive cycle 4)
- **cline (2026-09-13):** tester-soul.md second DEEP DIVE (production mutation + 2026 agent-test evidence): Google mutation playbook (arXiv:2103.07189 — 14.7M mutants, findings-not-scores, ≤7/file, 1 operator/line, 70% fault coupling on covered changes, Papadakis size-matched caveat), Meta ACH (arXiv:2501.12862 — mutant-guided LLM tests, 73% acceptance, equivalent-mutant agent 0.95/0.96), agent-test value counter-evidence (arXiv:2602.07900v2 — agent tests are process not proof; tester owns assertions), suite-evolution hazards (arXiv:2603.23443 SPC-residual >99%; arXiv:2212.11762 mutant brittleness 52%), Spotify honeycomb boundary, KB escape-classification template, MUTF verdict field. 6 tester additions + 10-row calibration table.
...
| cline | ✅ Complete | Deep Dive cycle 2 (engineer/routing/roadmap) + cycle 4 (tester-soul production mutation) |
...
| `OUTPUT/tester-soul.md` | ✅ Complete 2026-09-13 (freebuff deep dive + cline cycle-4 deep dive) | Full tester SOUL + DEEP DIVE (oracle quality, assertion lint, determinism) + DEEP DIVE cycle 4 (production mutation Google/Meta, agent-test value, suite evolution), 180 lines |
