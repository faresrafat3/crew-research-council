...
## [DEEP DIVE (freebuff, pass 3, 2026-09-14)]: GQM for the Ledger — Every Metric Must Trace to a Goal, and Safe Exploration for the Experiments That Produce Them

Pass 2 made the ledger *statistically readable* (p-charts, run rules). Missing: a discipline for deciding **which metrics deserve rows at all** and how to change the measurement system itself without gaming it. Both have established answers.

**Evidence.**
- **GQM (Basili's Goal-Question-Metric)**: define the goal first ("reduce defect escapes to production"), derive the questions that determine goal attainment ("which modules escaped?"), then collect only the metrics that answer those questions. Metrics without a goal lineage measure motion, not progress [Basili et al., GQM literature — established methodology].
- The gaming failure GQM prevents is exactly the crew's pass-2 Goodhart finding: a metric adopted because it is easy, not because it answers a goal question, will be optimized against — the pass-2 maturity ratify-or-revert rule is GQM's spirit applied to ladders.
- **Safe exploration**: changing thresholds or adding metrics is an experiment; the bandit/experiment literature's guardrail discipline applies — explore within guardrail bounds, never let an exploration arm violate a hard constraint (e.g., escape rate), and prefer Thompson sampling over ε-greedy for guardrail compliance because it allocates exploration proportionally to evidence [Thompson/bandit guardrail literature — snippet-verified 2026-09-14].

**Protocol deltas for the metrics ledger.**
1. **GQM lineage column becomes mandatory**: every ledger row records its goal and question. A row that cannot name its goal is deleted at the next quarterly review — deletion is the default, because each row costs attention (p-chart reading, pass 2).
2. **Threshold changes are experiments with guardrails**: proposed gate-threshold changes run in shadow (roadmap pass 2) against the ledger for one cycle before adoption, and never more than one threshold changes at once (attribution — same principle as the merge queue's bisection requirement, cicd pass 3).
3. **Thompson sampling for the improvement backlog**: candidate metric/threshold improvements are arms; exploration weight proportional to posterior evidence, with hard guardrails (escape rate, break-glass rate) excluded from exploration entirely.
4. **Ledger schema versioning**: the ledger itself gets a change log; a metric's history is not silently re-baselined when its definition changes (re-baselining is how dying metrics hide).

**Cross-links:** pass-2 SPC reading rules; maturity pass 3 (ledger deltas ratify levels); blocking-authority pass 3 (break-glass rate as a guardrail metric); cicd pass 3 (one-change-at-a-time attribution).

**Sources.**
1. [Basili, Caldiera & Rombach, 1994] "The Goal Question Metric Approach" [established methodology — literature].
2. [Basili et al., 2010+] GQM+Strategies: aligning measurement programs with business goals [literature].
3. [Thompson, 1933; agrawal & Goyal 2013] Thompson sampling — regret properties motivating guardrail-safe exploration [literature].
