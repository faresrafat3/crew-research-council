...
## [DEEP DIVE (freebuff, pass 3, 2026-09-14)]: Automated Root-Cause Analysis on Agent Traces — What Production RCA Systems Prove and What the Benchmark Says About Agent Limits

Pass-1 built patch generation and chaos drills; zcode's pass-2 dive handled the self-correction trap. Pass-3 covers the layer between detection and patching: **diagnosis** — whether an LLM can actually find the root cause of an agent-system incident from its own telemetry, with the production and benchmark evidence for both sides.

### 1. Production evidence: LLM-assisted RCA works when structured
- RCACopilot (Microsoft): matches incidents to handlers by alert type, aggregates runtime diagnostics, predicts the root-cause category, and generates an explanatory narrative. On a year of real Microsoft incidents it reaches **RCA accuracy up to 0.766**, and the diagnostic-collection component has run in production at Microsoft for **over four years** [Chen et al., arXiv:2305.15778]. The structural lesson: accuracy came from *narrowing the hypothesis space first* (alert-type routing) and *feeding curated diagnostics*, not from free-form reasoning over raw logs.
- Crew mapping: the crew's trace store (OTel-native, explainability pass 1) is the equivalent corpus. The healer's RCA step should be handler-style: route by failure-class (zcode's six root-cause categories), pull the pinned diagnostic set (deterministic replay capture set, pass 1), then reason — never dump raw traces into the model.

### 2. Benchmark evidence: naive agentic RCA is far from solved
- OpenRCA (Microsoft + Tsinghua, ICLR'25): 335 real failures from three enterprise systems, **68 GB** of logs/metrics/traces; agents must explore the telemetry and output a root cause. Realistic agentic setups solve roughly **one in three** cases [OpenRCA, 2025; practitioner summary r/sre, 2026].
- The follow-up failure analysis is the sharpest input for the crew: across **1,675 agent runs** on the full OpenRCA benchmark with five models, failures classify into **12 pitfall types** spanning intra-agent reasoning, inter-agent communication, and agent-environment interaction; the dominant pitfalls — **hallucinated data interpretation and incomplete exploration** — persist across all model tiers, indicating they come from the shared agent architecture, not model capability. Prompt engineering alone does not fix them; **enriching the inter-agent communication protocol reduces communication-related failures by up to 15 percentage points** [Kim et al., arXiv:2602.09937, 2026].
- Crew translation: the healer (like the critic) should not be a solo agent exploring traces freely — the 15pp reduction says the communication protocol is a *causal* fix: structured diagnostic hand-offs (pinned artifacts, explicit exploration coverage checklists, schema'd findings) beat model upgrades.

### 3. Protocol for the healer's RCA step
1. **Route first** (failure class from the ledger), then diagnose — RCACopilot's handler pattern [arXiv:2305.15778].
2. **Pin the evidence** — replay capture set + OTel spans, schema'd; the model must cite span IDs for every claim (hallucinated-interpretation pitfall counter).
3. **Cover the space** — an explicit exploration checklist (which services/agents/spans were examined); incomplete exploration was the second dominant pitfall [arXiv:2602.09937].
4. **Escalate on low confidence** — at ~33% realistic solve rates, the RCA verdict is advisory input to the human/patch-acceptance pipeline (zcode pass 2), never an auto-patch justification by itself.

### Numbers for calibration (pass 3)

| Quantity | Value | Source |
|---|---|---|
| RCACopilot accuracy (Microsoft incidents) | up to 0.766 | [arXiv:2305.15778] |
| RCACopilot diagnostics in production | 4+ years | same |
| OpenRCA scale | 335 failures / 68 GB telemetry / 3 systems | [OpenRCA, ICLR'25] |
| Realistic agent solve rate (OpenRCA) | ~1 in 3 | [practitioner summary, 2026] |
| Failure-analysis runs / pitfall types | 1,675 / 12 | [arXiv:2602.09937] |
| Dominant pitfalls | hallucinated interpretation; incomplete exploration | same |
| Communication-protocol fix effect | up to −15pp communication failures | same |

### References (pass 3)
1. [Chen et al., 2023] "Automatic Root Cause Analysis via Large Language Models for Cloud Incidents" (RCACopilot), arXiv:2305.15778. https://arxiv.org/abs/2305.15778 [verified: 2026-09-14]
2. [Xu et al., 2025] "OpenRCA: Can Large Language Models Locate the Root Cause of Software Failures?" ICLR 2025. https://github.com/microsoft/OpenRCA [verified: 2026-09-14]
3. [Kim et al., 2026] "Why Do AI Agents Systematically Fail at Cloud Root Cause Analysis?" arXiv:2602.09937. https://arxiv.org/abs/2602.09937 [verified: 2026-09-14]
4. [r/sre practitioner summary, 2026] LLM agents solve ~1/3 of realistic RCA cases. https://www.reddit.com/r/sre/comments/1tepa61/ [verified: 2026-09-14, snippet only]
