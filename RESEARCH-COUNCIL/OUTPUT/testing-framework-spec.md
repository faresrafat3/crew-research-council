...
## [DEEP DIVE (freebuff, pass 3, 2026-09-14)]: Snapshot, Golden-Master, and Characterization Tests — When the Lockstep Contract Is the Bug

The pass-1 spec (pytest config, property strategies, mutation operators) and the pass-2 coverage floor leave one layer unspecified: tests that assert *the output equals a stored artifact* rather than a property. Three variants circulate — snapshot testing (capture and compare serialized output), golden-master/approval testing (curated reference outputs), and characterization tests (pin whatever the code currently does during legacy refactoring). They share a failure mode the council must design against: **the test's oracle is the code's own past behavior, so any bug present at capture time is immortalized as the specification**.

**Evidence.**
- Characterization tests deliberately assert current behavior without judging it; their value is change detection during refactoring, not correctness [Feathers, *Working Effectively with Legacy Code*, ch. on characterization testing — literature].
- Snapshot tooling itself warns against trusting them: snapshots must be code-reviewed, are not self-evidently correct, and volatile fields (timestamps, ids, randomness, absolute paths) make them brittle [Jest snapshot docs; approval-testing literature — snippet-verified].
- The review-load problem compounds: a snapshot update is a diff of machine-generated text. The code-review size effect (see engineer-soul pass 3 — detection degrades beyond ~200–400 LOC) applies verbatim to snapshot diffs, which reviewers rubber-stamp at much higher rates than code.

**Protocol deltas for the framework spec.**
1. A golden artifact becomes part of the privileged test set only after **manual review of the first capture**; CI never auto-updates goldens (`--update` runs locally or in an explicitly labeled workflow, never in the merge path — ties to cicd pass 3).
2. **Volatility scrubbing is a capture prerequisite**: no wall-clock, random seeds, ids, or environment paths may reach the serialized artifact. A golden that needs re-scrubbing twice is rejected as an oracle.
3. Snapshot/golden diffs **count against the reviewer attention budget**; a PR whose golden update exceeds the SmartBear size ceiling must be split.
4. Where a property can be stated, the property-based layer wins: prefer hypothesis strategies over snapshots. Snapshots are the last resort for outputs nobody knows how to characterize.

**Cross-links:** engineer-soul pass 3 (review attention economics), tdd-protocol pass 3 (which tests survive refactors — characterization tests are deliberately refactor-hostile; keep them out of behavior suites), implementation-roadmap pass 3 (legacy hotspots are where characterization tests are legitimately deployed).

**Sources.**
1. [Feathers, 2004] *Working Effectively with Legacy Code* — characterization tests [literature].
2. [Jest, 2026] Snapshot testing docs: review requirement and volatility caveats. https://jestjs.io/docs/snapshot-testing [verified: 2026-09-14, snippet only]
3. [ApprovalTests, 2026] Approval testing documentation (golden-master semantics). https://approvaltests.com [verified: 2026-09-14, snippet only]
