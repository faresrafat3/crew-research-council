...
## [DEEP DIVE (freebuff, pass 3, 2026-09-14)]: Calibrated Abstention — Selective Prediction and Conformal Guarantees for the Escalation Gate

Pass-1 operationalized Article 14 and approval fatigue; Antigravity's pass-2 added decoy vigilance audits. Pass-3 gives the escalation gate a statistical backbone: the agent should escalate to the human not by fixed threshold, but by **abstaining when its own confidence fails a calibrated risk budget** — turning "escalate ambiguous cases" into a quantity with a guarantee.

### 1. The escalation gate is a selective predictor
- Selective classification: a model answers (auto-PROMOTE) or abstains (escalate). The two error directions are exactly HITL's: over-reliance = accepting incorrect agent output; under-reliance = escalating correct output, which spends the finite human budget (pass-1 fatigue model). The measurement constructs for appropriate reliance are established in the HCI literature [ACM, 2026; arXiv:2604.23896] — the crew's ledger already records both directions (FP rate and escape rate), so **appropriate reliance is measurable per policy change**, not just conceptual.
- The gap pass 1 left: thresholds move by calibration review (±5% steps), which tunes the *rate* of escalation but guarantees nothing about the *risk* carried by the non-escalated (auto-PROMOTED) share.

### 2. Conformal abstention: a guarantee instead of a threshold
- Conformal prediction provides **distribution-free coverage guarantees**: calibrate on held-out labeled tasks, then any selective policy that abstains on low-confidence cases carries a statistical bound on the error rate of the answered (non-escalated) set. Recent work applies exactly this to LLM settings — conformalized abstention policies for LLM/VLM risk control [CAP, OpenReview], selective conformal risk control [arXiv:2512.12844], hallucination-risk control via conformal methods [Royal Society scoping review, 2026], and conformal abstention with faithfulness guarantees for RAG [TMLS, 2026].
- Crew protocol: hold out each quarter's calibrated task set (ledger rows with human-verified verdicts); compute a nonconformity score per task (tester confidence × mutation margin × critic split indicator); pick the abstention threshold as the quantile that guarantees: *"the auto-PROMOTED stream's escape rate ≤ 1/20 at 95% confidence"* — the crew's existing escape target becomes the conformal risk level α. Escalation volume then lands wherever the guarantee requires, replacing the ad-hoc <5 escalations/week target with a derived one (and if the derived volume exceeds the fatigue budget, that is a *signal to change the system*, not to loosen the guarantee silently — the honest move is to raise α explicitly in TEST-POLICY, with the operator's sign-off).
- This also answers the EU AI Act Article 14 framing from pass 1: a documented, statistically bounded escalation policy is a stronger "human oversight effectiveness" artifact than a fixed threshold table.

### 3. Practical guards
- **Calibration drift:** recompute the quantile each quarterly cycle; monitor the empirical escape rate of the auto-PROMOTED stream on the SPC chart (quality-metrics pass 2) — a violation is a signal, and the response is recalibration, not threshold nudging.
- **Abstention asymmetry:** per routing pass 2 (FN ≫ FP), abstention should be cheap: the cost of an unnecessary escalation is bounded by the fatigue budget, while the cost of a wrong auto-PROMOTE is an escape — the conformal level is set on the escape side.
- **Distribution shift:** conformal guarantees are exchangeability-conditional; when the task distribution shifts (new formation, new domain), the calibration set is stale — the drift monitor (production pass 1) should force early recalibration.

### Numbers for calibration (pass 3)

| Quantity | Value | Source |
|---|---|---|
| Guarantee form | P(escape rate of auto-PROMOTED stream ≤ 1/20) ≥ 95% | this dive, conformal form |
| Calibration data | quarterly human-verified ledger rows | this dive |
| Threshold | α-quantile of nonconformity scores | [CAP; arXiv:2512.12844] |
| Recalibration trigger | quarterly + drift alarm | this dive |

### References (pass 3)
1. [Tayebati et al.] "CAP: Conformalized Abstention Policies for Context- and Risk-Aware LLM/VLM Agents." https://openreview.net/pdf?id=DA1ELJTudh [verified: 2026-09-14, snippet]
2. [2025/2026] "Selective Conformal Risk Control," arXiv:2512.12844. https://arxiv.org/html/2512.12844v2 [verified: 2026-09-14, snippet]
3. [Royal Society Phil. Trans., 2026] "Uncertainty-aware large language models: a scoping review" (conformal hallucination-risk control). https://royalsocietypublishing.org/rsta/article/384/2327/20260061/483104/ [verified: 2026-09-14, snippet]
4. [TMLS, 2026] "Conformal Abstention: Faithfulness Guarantees for RAG." https://www.tmls.nyc/research/conformal-abstention-rag [verified: 2026-09-14, snippet]
5. [ACM, 2026] "Measurement Constructs for Human-AI Appropriate Reliance." https://dl.acm.org/doi/10.1145/3772363.3798835 [verified: 2026-09-14, snippet]
6. Cross-refs: human-in-the-loop pass 1 (fatigue budget, Article 14); routing pass 2 (asymmetric error costs); quality-metrics pass 2 (SPC monitoring); production pass 1 (drift monitors).
