...
## [DEEP DIVE (freebuff, pass 3, 2026-09-14)]: Psychometric Validity Warning — Big Five Inventories Do Not Measure Personality in LLMs

Pass-1 covered persona vectors and collapse dynamics; pass-3 covers the measurement layer beneath the 5-dimension personality spec — with a 2026 result that directly challenges it.

### 1. The finding: the instrument does not transfer
- Zierahn et al. (AAAI/ACM AIES 2026) run a systematic psychometric evaluation of Big Five measurement in LLMs across five candidate inventories and **N = 264 LLMs from 50 families**. Three findings: (a) *human-developed items fail content validity for LLMs* (adapted items do better); (b) **between-model variance accounts for only 7–17% of total score variance** — the inventories largely do not capture meaningful differences between models; (c) LLM responses **do not reproduce the canonical five-factor structure — four of five facets collapse into one factor (r ≥ .90)**; and alignment training shifts scores toward socially desirable profiles [arXiv:2607.02325]. Their conclusion: using human personality frameworks to characterize, benchmark, compare, or govern LLMs "risks producing misleading conclusions."
- This lands on the base doc's 5-dimension personality model (and any voice-drift monitoring that reads it): the *dimensions may still be useful design vocabulary* — deliberate style choices with explicit examples — but **treating instrument scores as measurements of stable internal traits is not supported**. Test-retest stability of BFI-style scores in humans (r ≈ .75–.90 across months [standard psychometrics]) has no established LLM analog, and the collapse finding means the instrument's factor structure itself fails before stability can even be assessed.

### 2. What survives, and what replaces the score
- **Survives:** persona *vectors* (pass 1) — activation-space monitoring is a different measurement path (internal states, not self-report questionnaires) and is not implicated by the questionnaire critique; explicit behavioral specs in the SOUL ("voice" as style rules with few-shot exemplars) remain the enforceable form, per engineer-soul pass 2's verifiable-instruction architecture.
- **Replaces the score:** (a) behavioral audits — scripted scenario batteries scored by rubric (does the tester's output actually match the specified voice under varied phrasings — the cousin-prompt drill from engineer-soul pass 2, repurposed as the embodiment check); (b) the pass-1 functional-diversity metrics (Distinct-Perspective Count etc.) for *between-agent* differentiation rather than within-agent trait claims; (c) any "personality drift" alert must key on behavior deltas or activation-space deltas — never on questionnaire re-scores, which can shift with prompt phrasing without any behavioral change (the 7–17% finding).
- Governance note for the council: the base doc's "5-dimension personality" should carry a validity caveat pointing here, or downstream readers will instrument it.

### Numbers for calibration (pass 3)

| Quantity | Value | Source |
|---|---|---|
| Models assessed | N = 264, 50 families | [arXiv:2607.02325] |
| Between-model variance captured | 7–17% of score variance | same |
| Factor structure in LLMs | 4 of 5 facets collapse (r ≥ .90) | same |
| Alignment-training effect | shifts toward socially desirable profiles | same |
| Human BFI test-retest (contrast) | r ≈ .75–.90 | [standard psychometrics, snippet-level] |

### References (pass 3)
1. [Zierahn et al., 2026] "Personality Without Persons? A Psychometric Critique of Big Five Testing in Large Language Models," AAAI/ACM AIES 2026, arXiv:2607.02325. https://arxiv.org/abs/2607.02325 [verified: 2026-09-14]
2. [Human BFI reliability, contrast] BFI-10 retains 85% of retest reliability; standard instruments r ≈ .75–.90. [verified: 2026-09-14, secondary snippets]
3. Cross-refs: agent-embodiment pass 1 (persona vectors, collapse dynamics, functional diversity); engineer-soul pass 2 (cousin-prompt drills, verifiable instructions).
